...
export const updateUserAddress = (userId: string, data: UpdateAddressDto) =>
    axios.patch<UpdateAddressDto, ResponseEntity<UpdateAddressDto>>(`${BASE_URL}/${userId}/address`, data, {
        headers: {
            Authorization: `Bearer ${getToken()}`
        }});

export const getUser = (userId: string) =>
    axios.get<null, ResponseEntity<UserDto>>(`${BASE_URL}/${userId}`, {
        headers: {
            Authorization: `Bearer ${getToken()}`
        }});

export const updateUser = (userId: string, data: UpdateUserDto)  =>
    axios.patch<UpdateUserDto, ResponseEntity<UpdateUserDto>>(`${BASE_URL}/${userId}`, data, {
        headers: {
            Authorization: `Bearer ${getToken()}`
        }});